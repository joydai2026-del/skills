...
name: ai-done
description: >-
  The done bar for probabilistic AI / LLM / agent features: eval bands, failure triage, tripwires, rehearsed rollback, post-ship loop. Use when: "is this AI feature done", "ready to ship the model/agent", "calibrate this AI feature", "AI done bar", "how do I know this model/agent is ready", "done bar for a probabilistic feature", "ship checklist for an LLM feature". Non-AI deterministic features use /success-criteria; this runs after its checklist.
allowed-tools:
  - Read
  - Write
  - Edit
...
- Deterministic pass/fail checklist: it calls **/success-criteria**, then the checklist loop.
...
|---|---|---|
| L1 Deterministic floor | The non-AI parts pass hard pass/fail checks (auth, billing, nav, data integrity) plus the global QA base (static analysis, tests, device/real-env). | /success-criteria + the checklist loop; QA base verified directly |
...
The owner usually ships AI products as a one-person team on a serverless stack. L3 to L5 still apply, but they degrade honestly. Do NOT write your own name in every owner column and call it triage. Instead:
...
- **L5 "reviewer + cadence":** one person is fine; pin the cadence (for example "the owner, Friday, 15 sampled outputs + every flagged one") so it is a commitment, not a vibe.
...
1. Invoke **/success-criteria** on the deterministic list to generate or extend the machine-checkable checklist (it writes qa-checklist.yaml or the project's own convention; then loop the checklist). Record the checklist path in the calibration doc. Do NOT hand-write these checks here.
...
1. **Capture:** decide where production outputs (or a sample of them) are logged or stored (the app's request log, a database table, a storage volume, an LLM-observability tool).
...
5. **Outcome metric:** name the metric that proves customers actually succeeded, not just that the feature shipped (outputs are not outcomes). This is the house rule customer-need check applied after launch, not a separate gate.
...
Do not write it into `<NOTES_LINK>/` (that syncs to the notes app and is not the home for a per-feature ship artifact) unless the project has no `docs/` and no checklist dir.

Link the qa-checklist (L1) and the eval suite (L2); inline L3, L4, L5. Then, if `<AGENT_CONFIG>` (the house-rules file) carries an AI-features clause pointing here, confirm this calibration satisfies it. If the house rule lists only the deterministic checks with no AI clause, the five-layer bar in THIS skill is the AI definition of done; note that in the doc and proceed.
...
Never report "done" on a partial bar (a recurring failure pattern: check it against your own completion bar).
...
- **/success-criteria** (L1, sequential handoff in, not a competitor), the checklist loop (L1), **/eval-harness** (L2 suite + the suite L5 feeds), **/verify** (build / type / lint / test / security), a root-cause investigation pass (when a tripwire fires in production).
